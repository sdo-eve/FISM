...
pro gps_to_utc,gps,nleap,year,doy,utc,month,day,hour,min,sec,vern=vern,auto=auto,julian=julian
...
if size(leap_sec_ready_flag,/type) eq 0 then begin
    ref_file='tai-utc.dat'
    if file_test(ref_file) eq 0 then begin
        ref_file=expand_path('$fism_code') + '/idl/tai-utc.dat'
        if file_test(ref_file) eq 0 then begin
