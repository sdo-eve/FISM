...
pro concat_fism_daily_fuv, start_yd=start_yd, end_yd=end_yd, update=update

    print, 'Running concat_fism_daily.pro', !stime
    ...
    ; res_pth=expand_path('$daily_results') (-aw)
    res_pth=expand_path('$fism_results') + '/daily_data/fuv_daily_hr/'
    ...
    restore, expand_path('$fism_save') + '/best_proxy_fuv.sav'
    nwv=n_elements(best_tag[0,*])
    tmp_yd=start_yd
    print, end_yd, 'concat fuv'
    while tmp_yd le end_yd do begin
        ;print, 'FUV' + string(tmp_yd)
        best_sp=fltarr(nwv)
        ...
        svfl=res_pth+yr+'/FISM_daily_'+yd+'_fuv_'+version+'_hr.sav'
        day_ar=tmp_yd
        utc=0
        fism_pred=best_sp
        fism_wv=fism_wv_fuv
		...
		if getenv('$gen_daily_ascii') eq 'true'  then begin
			...
			out_file = res_pth + yr + '/FISM_daily_' + yd + '_fuv_' + version + $
				'.txt'
...
print, 'End Time concat_fism_daily_fuv: ', !stime
