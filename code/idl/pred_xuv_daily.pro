...
function pred_xuv_daily, yyyydoy, utc, no_scav=no_scav, sr_coef=sr_coef, $
...
for j=0,nwv-1 do begin
   ...
   ;FISM Pred
	...
	;pred_eve_dmin_sc[j]=fit_coefs_sc2[0,j]+1.; +1 takes care of the min sub in numerator before dividing
	...
	;for k=0,nx-1 do begin
		...
		pred_eve_dmin_sc[j]=pred_eve_dmin_sc[j]+(fit_coefs_sc[1,j]*x_fit_sc)
                                ; Force to be greater than 1.0 to
                                ; always be greater than SC min
                ;pred_eve_dmin_sc[j]=(pred_eve_dmin_sc[j]+(fit_coefs_sc2[1,j]*x_fit_sc)+(fit_coefs_sc2[2,j]*x_fit_sc*x_fit_sc))>1.0 
...
pred_sp_sc=mult_min_xuv(pred_eve_dmin_sc)
pred_sp_sr=mult_min_xuv(pred_eve_dmin_sr)
