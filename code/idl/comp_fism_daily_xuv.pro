...
pro comp_fism_daily_xuv, start_yd=start_yd, end_yd=end_yd, scsr_split=scsr_split, $
...
if not keyword_set(start_yd) and keyword_set(preerr) then start_yd=2003001 $ ; Prior to SORCE launch
...
common goes_data_com, goes, goes_pri, ychk, day_ar_all, coefs, p_mgii, p_f107, p_goes, $
	...
	fit_coefs_sc, fit_coefs_sc2, def_tag, backup_ar, err_tag, nwv, $
...
nwv=400
fism_wv=findgen(nwv)/10.+0.05 ; Set the FISM XUV wavelength scale
...
coef_fl= expand_path('$fism_save') +'/sc_sr_fit_coefs_xuv_tag'+strtrim(tag,2)+'.sav'
restore, coef_fl
...
if not keyword_set(preerr) then restore, expand_path('$fism_save') + '/fism_daily_error_xuv.sav'
...
while tmp_yd le end_yd do begin
	...
	for j=0l,end_sod do begin
		...
		if keyword_set(sr_coef) then begin
			fism_pred_tmp=pred_xuv_daily(tmp_yd,43200, $
			...
			fism_pred_tmp=pred_xuv_daily(tmp_yd,43200, $
				tag=tag, /daily) ; 		
                     endelse
                                ;fism_pred_tmp=fism_pred_tmp>0.0
                ; Eliminate any points that are <0
                bdfsm=where(fism_pred_tmp lt 0.0)
                fism_pred_tmp[bdfsm]=0.0
	...
	if not keyword_set(preerr) then begin
           fism_err_daily = reform(fism_sig_xuv[err_tag-1, *])
           fism_err_daily_abs = reform(fism_sig_abs_xuv[err_tag-1 , *])
	endif
...
if next_tag[0] ne -1 then begin
	...
	remain_tag[wtag]=-1
	print, 'Tag number begin processed, XUV: '+strtrim(tag,2)+'  ', !stime
endif else brk=1

endwhile	; End while loop for each tag

print, 'Start Time: ', st
print, 'End Time comp_fism_daily_xuv: ', !stime
