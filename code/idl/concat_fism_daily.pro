...
pro concat_fism_daily, start_yd=start_yd, end_yd=end_yd, update=update

    FILE_MKDIR, expand_path('$fism_results') + '/daily_data/'
    ...
    restore, expand_path('$fism_save') + '/best_proxy.sav'
    ...
    while tmp_yd le end_yd do begin
        ...
        while brk ne 1 do begin
            pred_fl='FISM_tmp_daily_'+strtrim(tmp_yd,2)+'_tag'+ $
                strtrim(tag,2)+'.sav'
        ...
        endwhile
        svfl=res_pth+yr+'/FISM_daily_'+yd+'_'+version+'.sav'
        day_ar=tmp_yd
        utc=0
        fism_pred=best_sp
		...
		if getenv('$gen_daily_ascii') eq 'true'  then begin
			...
			out_file = res_pth + yr + '/FISM_daily_' + yd + '_' + version + $
				'.txt'
...
print, 'End Time concat_fism_daily: ', !stime
