...
pro make_backup_best_proxy, plots=plots, ps_out=ps_out

print, 'Running make_backuo_best_proxy ', !stime


restore, expand_path('$fism_save')+'/best_fit_coefs.sav'
...
save, best_tag, file=expand_path('$fism_save')+'/best_proxy.sav'

if keyword_set(plots) or keyword_set(ps_out) then begin
	if keyword_set(ps_out) then open_ps, '$fism_plots/backup_proxy.ps',/landscape,/color
...
print, 'End Time make_backup_best_proxy: ', !stime
