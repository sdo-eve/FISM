...
pro an_fit_coefs, debug=debug, manual_sel=manual_sel

print, 'Running an_fit_coefs ', !stime
...
restore, save_pth+'/three_prox_fitcoefs_tag'+strtrim(tag_ar[0],2)+'_noscav.sav'
...
for i=0,ntags-1 do begin
	...
	flnm='$fism_save/three_prox_fitcoefs_tag'+strtrim(tag_ar[i],2)+'_noscav.sav'
	restore, flnm
...
if keyword_set(manual_sel) then begin
   ...
   for j=0,nwv-1 do begin
      ...
      if best_tag[j] eq 3 then best_tag[j] =1
      ;if best_tag[j] eq 10 then best_tag[j] = 6 ; LyaD not working, use Lya
   endfor
endelse

if not keyword_set(debug) then save, best_tag, nwv, file=save_pth+'/best_fit_coefs.sav' $
                                     else stop

print, 'End Time an_fit_coefs: ', !stime
